...
function wire(){
 document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!$('#modalBackdrop').hidden)closeAll()});
 $('#search').addEventListener('input',e=>{state.query=e.target.value.toLowerCase().trim();renderMenu()});
 $('#openCart').addEventListener('click',()=>openSheet('cart'));$('[data-action="cart"]').addEventListener('click',()=>openSheet('cart'));$('#mobileCart').addEventListener('click',()=>openSheet('cart'));
...
function openProduct(id){const p=product(id);state.current=p;state.currentConfig={qty:1,removed:[],extras:[],burgers:[],drinks:[],notes:''};$('#modalTitle').textContent=p.name;$('#modalBody').innerHTML=modalContent(p);updateModalTotal();openSheet('product');wireModal(p);pushEvent('view_item',{item_id:p.id,item_name:p.name,value:p.price});}
...
function commitCurrent(){const err=validateCurrent();if(err)return toast(err);const p=state.current,c=structuredClone(state.currentConfig);state.cart.push({key:crypto.randomUUID?.()||String(Date.now()+Math.random()),id:p.id,qty:c.qty,unitPrice:p.price,config:c});save();renderCart();closeAll();toast('Adicionado à sacola');pushEvent('add_to_cart',{item_id:p.id,item_name:p.name,value:p.price*c.qty,quantity:c.qty});}
...
function renderCart(){const n=state.cart.reduce((s,l)=>s+l.qty,0);$('#cartCount').textContent=n;$('#mobileCartCount').textContent=n;$('#cartTotal').textContent=money(cartTotal());$('#mobileCartTotal').textContent=money(cartTotal());if(!state.cart.length){$('#cartBody').innerHTML='<div style="text-align:center;color:#888;padding:50px 10px">Sua sacola está vazia.<br><br>Escolha algo que dê vontade de pedir de novo.</div>';return}$('#cartBody').innerHTML=state.cart.map(l=>{const p=product(l.id);return `<div class="cart-line"><div><strong>${p.name}</strong><div class="cart-line__meta">${itemMeta(l)||'Sem personalizações'}</div><div class="qty-row" style="margin-top:10px"><button type="button" data-dec="${l.key}">−</button><span>${l.qty}</span><button type="button" data-inc="${l.key}">+</button></div></div><strong>${money(l.unitPrice*l.qty)}</strong></div>`}).join('')+crossSell();$$('[data-dec]').forEach(b=>b.onclick=()=>changeCart(b.dataset.dec,-1));$$('[data-inc]').forEach(b=>b.onclick=()=>changeCart(b.dataset.inc,1));$$('[data-cross]').forEach(b=>b.onclick=()=>openProduct(b.dataset.cross));}
...
function closeAll(hideBackdrop=true){['#productModal','#cartDrawer','#checkoutDrawer'].forEach(s=>$(s).hidden=true);if(hideBackdrop)$('#modalBackdrop').hidden=true;document.body.classList.remove('lock')}
